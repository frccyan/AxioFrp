...
func main() {
	...
	configPath := flag.String("config", filepath.Join(configDir, "config.ini"), "配置文件路径")
	mode := flag.String("mode", "both", "模式: generate(仅生成配置), run(仅运行frpc), both(生成配置并运行)")
	frpcPath := flag.String("frpc", filepath.Join(configDir, "frpc"), "frpc可执行文件路径")
	...
	// 优先从环境变量获取token
	envToken := os.Getenv("FRP_TOKEN")
	if envToken != "" {
		tokenInput = envToken
		fmt.Printf("已使用环境变量中的访问密钥\n")
		
		// 保存环境变量中的token到文件
		if err := ioutil.WriteFile(tokenFile, []byte(tokenInput), 0644); err != nil {
			fmt.Printf("保存环境变量中的访问密钥失败: %v\n", err)
		}
	} else if *tokenFlag != "" {
		// 其次使用命令行参数中的token
	...
	// 使用命令行参数覆盖frpc路径
	if *frpcPath != filepath.Join(configDir, "frpc") {
	...
	// 处理节点选择
	nodeFile := filepath.Join(configDir, "node.txt")
	
	// 1. 优先使用环境变量中的节点
	envNodeStr := os.Getenv("FRP_NODE")
	if envNodeStr != "" {
		fmt.Printf("使用环境变量指定的节点: %s\n", envNodeStr)
		selectedNodeID = parseNodeIdentifier(db, envNodeStr)
		// 保存选择的节点到文件
		if selectedNodeID > 0 {
		...
		// 如果没有指定节点或者指定的节点无效，则列出所有可用节点供用户选择
		if selectedNodeID <= 0 {
	...
	if *mode == "generate" || *mode == "both" {
		// 检查frpc.toml文件是否存在
		mainConfigPath := filepath.Join(configDir, "frpc.toml")
		...
		// 生成配置文件
		configFile := filepath.Join(configDir, fmt.Sprintf("frpc_%d.toml", selectedNodeID))
		...
		err = ioutil.WriteFile(mainConfigPath, configData, 0644)
		if err != nil {
...
RunPart:
	...
	if *mode == "run" || *mode == "both" {
		...
		cmd := exec.Command(config.FrpcPath, "-c", mainConfigPath)
		...
		// 在Docker中运行时，等待frpc运行直到程序结束
		fmt.Println("\nfrpc客户端正在运行，按Ctrl+C停止程序...")
		
		// 等待frpc进程结束
		err = cmd.Wait()
		if err != nil {
			log.Fatalf("frpc客户端异常退出: %v", err)
		}
	} else {
		// 如果只是生成配置，显示提示信息
		fmt.Println("\n配置文件已生成，可以使用以下命令运行frpc:")
		fmt.Printf("%s -c %s\n", config.FrpcPath, filepath.Join(configDir, "frpc.toml"))
		
		// 自动执行
		if *autoRun {
			fmt.Printf("\n正在自动执行: %s -c %s\n", config.FrpcPath, filepath.Join(configDir, "frpc.toml"))
			cmd := exec.Command(config.FrpcPath, "-c", filepath.Join(configDir, "frpc.toml"))
			cmd.Stdout = os.Stdout
			cmd.Stderr = os.Stderr
			err = cmd.Start()
			if err != nil {
