        ...
        <div class="row">
                    ...
                    <div class="card-body p-0 table-responsive">
						<table class="table table-bordered table-hover dataTable dtr-inline" style="width: 100%;font-size: 15px;">
							<tr>
								...
								<th class='text-center' nowrap>节点名称</th>
								<th class='text-center' nowrap>主机名</th>
								<th class='text-center' nowrap>IP 地址</th>
							...
							foreach($rs as $node) {
								$i++;
								$statuss = Array(
									200 => "<span style='color:green;'><i class='fas fa-check-circle'></i> 正常</span>", 
									403 => "<span style='color:red;'><i class='fas fa-ban'></i> 禁用</span>", 
									500 => "<span style='color:red;'><i class='fas fa-exclamation-circle'></i> 离线</span>", 
									401 => "<span style='color:gray;'><i class='fas fa-eye-slash'></i> 隐藏</span>"
								);
								$status  = $statuss[Intval($node[10])] ?? "<span style='color:orange;'><i class='fas fa-question-circle'></i> 未知</span>";
								echo "<tr>
								...
								<td class='text-center' nowrap>
									<a href='javascript:edit({$node[0]})' class='btn btn-sm btn-info'><i class='fas fa-edit'></i> 编辑</a> 
									<a href='javascript:deletenode({$node[0]})' class='btn btn-sm btn-danger'><i class='fas fa-trash'></i> 删除</a>
								</td>
								";
							}
							?>
						...
						<?php
						if($i == 0) {
							echo "<div class='alert alert-warning m-3'><i class='fas fa-info-circle'></i> 没有找到符合条件的结果</div>";
						}
						?>
					</div>
				</div>
			...
			<div class="col-lg-4">
                <div class="card">
                    <div class="card-header border-0">
                        <div class="d-flex justify-content-between">
                            <h3 class="card-title"><i class="fas fa-server"></i> 添加或修改节点信息</h3>
                        </div>
                    </div>
                    <div class="card-body">
						<p id="statusmsg" class="alert alert-info"><i class="fas fa-info-circle"></i> 点击左侧节点列表的编辑按钮来编辑节点信息</p>
						...
						<p><b>IP 地址</b>&nbsp;&nbsp;<small>服务器的 IP 地址或域名</small></p>
					...
					<div class="card-footer">
						<button class="btn btn-primary float-sm-right" onclick="save()"><i class="fas fa-save"></i> 保存设置</button>
					</div>
				</div>
			</div>
		</div>
	...
	var htmlobj = $.ajax({
		...
		success: function() {
			try {
				...
				$("#statusmsg").html("<i class='fas fa-edit'></i> 正在编辑节点 <b>" + json.name + "</b> 的设置");
