...
class ProxyManager {
	...
	public function isCustomDomainAvailable($domain, $proxy_type, $node)
	{
		return Database::querySingleLine("proxies", Array("customdomains" => $domain,
			"proxy_type" => $proxy_type, "node" => $node)) ? false : true;
	}
	...
	public function checkRules($data)
	{
		...
		$_list = Array("node", "proxy_name", "proxy_type", "local_ip", "local_port", "remote_port");
		...
		if($data['proxy_type'] !== "http" && $data['proxy_type'] !== "https") {
			...
			$hasDomain       = isset($data['domain']) && $data["domain"] !== "";
			$hasCustomDomain = isset($data['customdomains']) && $data["customdomains"] !== "";

			if(!$hasDomain && !$hasCustomDomain) {
				return Array(false, "创建 HTTP 或 HTTPS 类型的隧道时必须要指定一个自定义域名或内置域名");
			}

			if($hasDomain && $hasCustomDomain) {
				return Array(false, "自定义域名和内置域名只能填写其一");
			}
			...
			if($hasCustomDomain) {
				if(!$this->isCustomDomainAvailable($data['customdomains'], $data['proxy_type'], $data['node'])) {
					return Array(false, "此域名已经被其他隧道使用，同一协议（http/https）一个域名只能添加一次。");
				}
				if(!Regex::isDomain($data['customdomains'])) {
					return Array(false, "域名格式错误，请输入一个有效并且真实存在的域名");
				}
			}
		}
...
EOF;
			...
			if($item[3] == "http" || $item[3] == "https") {
				$domains = [];
				$customDomain = $item[17] ?? "";
				if($customDomain !== "") {
					$domains = Array($customDomain);
				} elseif(!empty($item[8])) {
					$decoded = json_decode($item[8], true);
					if(is_array($decoded)) {
						$domains = $decoded;
					} else {
						$domains = Array($item[8]);
					}
				}
				if(count($domains) > 0) {
					$configuration .= 'customDomains = ["' . $domains[0] . "\"]\n";
				}
				...
				$configuration .= $item[12] == "" ? "" : "transport.authKey = \"{$item[12]}\"\n";
			}
			
			// 压缩和加密
			$configuration .= $item[6] == "" ? "" : "transport.useEncryption = {$item[6]}\n";
			$configuration .= $item[7] == "" ? "" : "transport.useCompression = {$item[7]}\n";
			$configuration .= "\n";
		}
