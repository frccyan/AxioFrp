...
class UserManager {
	...
	public function sendFindpassEmail($username, $email, $link)
	{
		...
		$type  = chhcn\Utils::isHttps() ? "https" : "http";
	...
	public function decreaseTraffic($username, $traffic)
	{
		$user_info = $this->getInfoByUser($username);
		if ($user_info) {
			$new_traffic = $user_info['traffic'] - $traffic;
			if ($new_traffic < 0) {
				$new_traffic = 0;
			}
			Database::update("users", ["traffic" => $new_traffic], ["username" => $username]);
			return true;
		}
		return false;
	}
	...
	public function updateUser($id, $data)
	{
		...
		if(is_array($rs)) {
			...
			if(isset($data['inbound'], $data['outbound']) && chhcn\Regex::isNumber($data['inbound']) && chhcn\Regex::isNumber($data['outbound'])) {
