...
AxioFrp 提供了强大的管理后台，让管理员可以通过Web界面轻松管理系统配置：

### 📋 管理功能

- **🌐 系统设置**: 在线配置邮件、JWT、用户限制等参数
- **📊 系统状态**: 实时监控系统性能和服务状态  
- **🔧 邮件配置**: 支持多种邮件服务商，可在线测试
- **👥 用户管理**: 设置用户限制、默认套餐等
- **📈 配置历史**: 追踪配置变更，支持一键回滚
- **🔄 服务管理**: 重启服务、清理缓存等运维操作
- **📥 配置导入导出**: 批量管理和备份配置
...
登录系统后，管理员可通过以下路径访问：
- **系统设置**: `http://your-domain/admin/settings`
- **系统状态**: `http://your-domain/admin/system-status`

### 🔐 权限控制

- 只有具有 `admin` 角色的用户才能访问管理后台
- 非管理员用户访问时会显示403权限不足页面
- 所有管理操作都有完整的权限验证
...
| GET | `/api/config/system-status` | 获取系统状态 | 👑 |
